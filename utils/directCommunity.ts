...
export interface DirectCommunityInput {
    precio_min?: number;
    precio_max?: number;
    precio?: number;
    valor_noche?: number;
...
export function isLodgingCategory(label: string | undefined): boolean {
...
/**
 * Computes verifiable direct-to-host COP range from pricing metadata.
 * Returns null when no explicit host share is documented (anti-greenwashing).
 */
export function computeDirectCommunityAmount(
    ...
    const candidates = items.filter((item) => {
        ...
        if (options?.lodgingOnly && 'categoria' in item) {
            return isLodgingCategory((item as { categoria?: string }).categoria);
        }
        return true;
    });
...
export function formatDirectCommunityRange(amount: DirectCommunityAmount): string {
    if (amount.minCop === amount.maxCop) return formatCop(amount.minCop);
    return `${formatCop(amount.minCop)} – ${formatCop(amount.maxCop)}`;
}

/** Refugio object shape (`desglose_tarifas` + optional `porcentaje_anfitrion`). */
...
/** Destination `pricingGuide` array items (lodging rows with verified host share). */
export function computeDirectCommunityFromPricingGuide(
